...
<head>
    ...
    <title>Silt: {{ $tag->name }} | Minu Blogi</title>
...
<body class="bg-base-200">
    ...
    <div class="animated-gradient text-white py-24">
        <div class="container mx-auto px-4 text-center">
            <div class="badge badge-lg bg-white/20 text-white border-none mb-4">🏷️ Silt</div>
            <h1 class="text-5xl md:text-6xl font-bold mb-4">#{{ $tag->name }}</h1>
            <p class="text-xl opacity-90">{{ $posts->count() }} postitust selle sildiga</p>
        </div>
    </div>

    <div class="container mx-auto px-4 py-16 max-w-4xl">
        <div class="mb-8">
            <a href="/" class="btn btn-ghost btn-sm">← Tagasi avalehele</a>
        </div>
                ...
                @foreach($posts as $post)
                    <div class="blog-card card bg-base-100 shadow-xl">
                        <div class="card-body">
                            ...
                            <p class="text-base-content/70 mt-2">{{ Str::limit($post->content, 150) }}</p>
                            <div class="card-actions justify-between items-center mt-4">
                                <div class="flex gap-3">
                                    <span class="btn btn-sm btn-ghost">💬 {{ $post->comments_count }}</span>
                                    <span class="btn btn-sm btn-ghost">❤️ {{ $post->likes_count }}</span>
                                </div>
        ...
        @else
            <div class="text-center py-16">
                ...
                <p class="text-xl">Selle sildiga pole veel postitusi.</p>
